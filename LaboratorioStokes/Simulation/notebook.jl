...
# ╔═╡ 14236a30-50fc-11ee-3bc2-edd08297c199
begin
	#Max Garcia :)
	#Import Packages
	using Markdown
...
# ╔═╡ e1a86431-10c5-4f95-9188-0cee6f543de8
html"""<style>
main {
    max-width: 1050px;
}
...
# ╔═╡ e06abfc7-3d8e-4499-b169-63a52151f941
md"""
# F3001C- Vector Beams Laboratory
"""

# ╔═╡ 4d368e73-6894-43a1-b1d3-db5ed2b18947
TableOfContents(title="Table of Contents", aside=true)

# ╔═╡ a1482509-ba2a-478d-86a4-ef4b6a94e96d
md"""
## Numerical functions
"""

# ╔═╡ f5ec3c2e-f35b-489b-91d2-49cc2cc3fa86
md"""
Definition of: polarizers, waveplates and polarization visualization.
"""

# ╔═╡ aeddc459-3d03-42eb-aec5-844ea5c53375
begin
	"""
...
# ╔═╡ d609ef70-b2c6-49db-be0e-410165af5f9e
begin
	"""
...
# ╔═╡ d094ea81-d148-4960-87ce-3b32acb1bb6b
begin
	"""
...
# ╔═╡ a81bcffe-93f5-43dd-8556-e9991e416c40
begin
	"""
		qPlate(Ex, Ey, q)
	
	Jones matrix for a polarizer in cartesian basis	"""
	function qPlate(Ux,Uy,Ex, Ey, q)
	    #Jqplate = [cos(theta*2*q) sin(theta*2*q);sin(theta*2*q) -cos(theta*2*q)]
		Ym=Uy.*(ones(size(Ux)))
		Xm=Ux.*(ones(size(Uy)))
		theta = atan.(Ym./Xm)
	    EX = cos.(theta*2*q).*Ex .+ sin.(theta*2*q).*Ey
	    EY = sin.(theta*2*q).*Ex .- cos.(theta*2*q).*Ey
	    return EX, EY
	end
 end

# ╔═╡ d4f1f9dc-ecd9-4fda-899b-5f474e6ce2e6
md"""
Then, we define the beam function
"""

# ╔═╡ 2bed3481-ce7f-4b15-a29f-a71a87a304da
begin
	"""
...
# ╔═╡ 2e04f92c-7fe9-4c2e-a6a2-ad541344d668
begin
...
# ╔═╡ 5b22b122-760d-43cb-8945-2d1c5d50fcaf
md"""
## Numerical window definition
"""

# ╔═╡ bb15fa6b-b2bb-4963-95a9-277098cc07f9
begin
...
# ╔═╡ 51e56a56-0a15-4d1b-9495-4ed0f7ff08a6
md"""
## Beam generation
...
# ╔═╡ 847324a2-bba2-4db9-8f47-fbfc094ef87e
U = LaguerreGaussBeam.(Xs, Ys, 0.0, w0, 0.0, 633E-9, 0, 0);

# ╔═╡ 79fbbaff-5a84-4182-b48e-4bf858db6317
md"""
### Polarization basis definition
"""

# ╔═╡ f7e8ae76-e6eb-48fc-8e51-7a8959e1c0bc
begin
    """Define Polarization
...
# ╔═╡ 82ab122d-622b-4255-b70c-7fbbb4c5e4fe
md"""
## Numerical experiments
"""

# ╔═╡ 1bbb62a8-dd42-43a9-af83-94307c6b9fa1
md"""
### Experiment simulation
"""

# ╔═╡ f56820aa-6184-4c7b-9462-11343a98e00b
md"""
Polarizer $(@bind checkP CheckBox()) Angle $(@bind thpold Slider(0:15:90, default=0.0, show_value=true))

Before Retarder 1/2 $(@bind checkB2 CheckBox()) 1/4 $(@bind checkB4 CheckBox()) Angle $(@bind thBd Slider(0:15:90, default=0.0, show_value=true))

After Retarder 1/2 $(@bind checkA2 CheckBox()) 1/4 $(@bind checkA4 CheckBox()) Angle $(@bind thAd Slider(0:15:90, default=0.0, show_value=true))
"""

# ╔═╡ 59ec4830-c95c-49df-b18a-9bec5b479c5b
begin
	thA = (thAd-90)/180*pi
	thB = (thBd-90)/180*pi
	th = (thpold-90)/180*pi
	Eout = [Ex,Ey]
	if checkB2
		Eout = Halfwaveplate(Eout[1], Eout[2], thB)
	end
	if checkB4
		Eout = Quarterwaveplate(Eout[1], Eout[2], thB)
	end
	Eout = qPlate(Xs, Ys, Eout[1], Eout[2], 1/2)
	if checkA2
		Eout = Halfwaveplate(Eout[1], Eout[2], thA)
	end
	if checkA4
		Eout = Quarterwaveplate(Eout[1], Eout[2], thA)
	end
	if checkP
		Eout = Polarizer(Eout[1], Eout[2], th)
	end
	Ex1 = Eout[1]
	Ey1 = Eout[2]
end;

# ╔═╡ 4aeee2f0-062a-4074-ba64-ed5ff8c0bc3d
begin
	plotly()
	...
	if checkP
		...
		psetup1 = Plots.plot3d!([-1.0*cos(th), 1.0*cos(th)], [-5*[1,1]], [-1.0*sin(th), 1.0*sin(th)], linewidth=2.0, color=:white)
	end
	if checkB2
		psetup6 = Plots.plot3d!(1.0.*cos.(thcoordinate), [-6.0*ones(length(thcoordinate))], 1.0.*sin.(thcoordinate), linewidth=2.0, color=:blue)
		psetup6 = Plots.plot3d!([-1.0*cos(thB), 1.0*cos(thB)], [-6*[1,1]], [-1.0*sin(thB), 1.0*sin(thB)], linewidth=2.0, color=:blue)
	end
	if checkB4
		psetup6 = Plots.plot3d!(1.0.*cos.(thcoordinate), [-6.0*ones(length(thcoordinate))], 1.0.*sin.(thcoordinate), linewidth=2.0, color=:yellow)
		psetup6 = Plots.plot3d!([-1.0*cos(thB), 1.0*cos(thB)], [-6*[1,1]], [-1.0*sin(thB), 1.0*sin(thB)], linewidth=2.0, color=:yellow)
	end
	if checkA2
		psetup6 = Plots.plot3d!(1.0.*cos.(thcoordinate), [-4.0*ones(length(thcoordinate))], 1.0.*sin.(thcoordinate), linewidth=2.0, color=:blue)
		psetup6 = Plots.plot3d!([-1.0*cos(thA), 1.0*cos(thA)], [-4*[1,1]], [-1.0*sin(thA), 1.0*sin(thA)], linewidth=2.0, color=:blue)
	end
	if checkA4
		psetup6 = Plots.plot3d!(1.0.*cos.(thcoordinate), [-4.0*ones(length(thcoordinate))], 1.0.*sin.(thcoordinate), linewidth=2.0, color=:yellow)
		psetup6 = Plots.plot3d!([-1.0*cos(thA), 1.0*cos(thA)], [-4*[1,1]], [-1.0*sin(thA), 1.0*sin(thA)], linewidth=2.0, color=:yellow)
	end
...
# ╔═╡ 1b85a898-078e-4155-8eec-9932924bbb61
begin
	#Ih = 1
	#Iv = 0.5
	#Id = 0.75
	...
	#A = 1/2 * (S0+sqrt(S1^2+S2^2))	#Semi-major axis
	#B = 1/2 * (S0-sqrt(S1^2+S2^2))	#Semi-minor axis
	#Phi = -atan(S2/S1)              #Angle
	#HD = sign(S3)					#Handedness
	
	#P = sqrt(S1^2+S2^2+S3^2)/S0 	#Polarization grade
end
...
julia_version = "1.9.3"
manifest_format = "2.0"
project_hash = "95ab4005f5249157cf365bd7343e48b988a50207"
